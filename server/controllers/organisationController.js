...
export const createOrganisationAdmin = catchAsyncError(async (req, res, next) => {
    ...
    let organisation = await organisationModel.findOne({organisation_email: email});

    if(organisation) {
        return next(new ErrorHandler("ORGANISATION ALREADY EXISTS.", 409));
    }

    let lastOrgDoc = await organisationModel.find().sort({_id: -1}).limit(1);
...
export const organisationLogin = catchAsyncError(async (req, res, next) => {
    ...
    const organisation = await organisationModel.findOne({ organisation_id: id }).select("+organisation_password");
    ...
    sendToken(res, organisation, "ORGANISATION LOGGED IN SUCCESSFULLY", 200);
});
...
export const updateOrganisationAdmin = catchAsyncError( async (req, res, next) => {

    const { name, address, email, phone } = req.body;
    const file = req.file;

    let organisation = await organisationModel.findById(req.organisation._id);
    ...
    if(file) {
        const fileURI = getDataURI(file);
        const mycloud = await cloudinary.v2.uploader.upload(fileURI.content);
        await cloudinary.v2.uploader.destroy(organisation.organisation_logo.public_id);
    ...
    organisation = await organisationModel.findByIdAndUpdate(organisation._id, updatedOrgData, {new:true, runValidators:true});
...
export const addMemberAdmin = catchAsyncError(async (req, res, next) => {
    ...
    if(!organisation) {
        return next(new ErrorHandler("YOU ARE NOT LOGGED IN OR YOU DO NOT HAVE THE PERMISSION.", 400));
    }
    ...
    if(!orgNum) {
        return next(new ErrorHandler("ORGANISATION WITH THE GIVEN ID DOES NOT EXIST", 401));
    }
    ...
    let lastUserOfOrg = await userModel.find({organisation_id}).sort({_id: -1}).limit(1);
...
export const updateOrganisationPasswordAdmin = catchAsyncError( async (req, res, next) => {
    ...
    const organisation = await organisationModel.findById(req.organisation._id).select("+organisation_password");
...
export const removeMemberAdmin = catchAsyncError( async(req, res, next) => {
    ...
    if(!member) {
        return next(new ErrorHandler("MEMBER WITH THE GIVEN EMAIL DOES NOT EXIST.", 409));
    }
...
export const searchSingleMemberWithId = catchAsyncError(async (req, res, next) => {
    const { id } = req.body;
    if(!id) {
        return next(new ErrorHandler("PLEASE ENTER ALL FIELDS.", 401));
    }
    ...
    if(!member) {
        return next(new ErrorHandler("MEMBER WITH THE GIVEN ID DOES NOT EXIST", 401));
    }
...
export const getSingleMemberDetails = catchAsyncError( async(req, res, next) => {
    const member = await userModel.findById(req.params.id);
    if(!member) {
        return next(new ErrorHandler("MEMBER WITH THE ID DOES NOT EXIST", 401));
    }
    res.status(200).json({
        success: true,
        member
...
export const deleteOrganisationAdmin = catchAsyncError(async (req, res, next) => {
...
export const organisationForgotPasswordAdmin = catchAsyncError(async(req, res, next) => {
    const { email } = req.body;
    ...
    const organisation = await organisationModel.findOne({organisation_email: email});

    if(!organisation) {
        return next(new ErrorHandler("NO ORGANISATION WITH THIS EMAIL EXISTS", 400));
    }
...
export const organisationResetPasswordAdmin = catchAsyncError(async (req, res, next) => {
    const { token } = req.params;
    const resetPasswordToken = crypto.createHash("sha256").update(token).digest("hex");
    const organisation = await organisationModel.findOne({organisation_resetPasswordToken: resetPasswordToken, organisation_resetPasswordExpire: {$gt: Date.now()}});
