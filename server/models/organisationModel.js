...
organisationSchema.pre("save", async function(next) {
    ...
    this.organisation_password = await bcrypt.hash(this.organisation_password, 10);
    next();
})

organisationSchema.methods.getJwtToken = function() {
    return jwt.sign({ _id: this._id, _type: "organisation" }, process.env.JWT_SECRET, { expiresIn: "3d" });
}

organisationSchema.methods.comparePassword = async function(organisation_password) {
    return await bcrypt.compare(organisation_password, this.organisation_password);
}

organisationSchema.methods.getResetToken = function() {
    const resetToken = crypto.randomBytes(20).toString("hex");
    this.organisation_resetPasswordToken = crypto.createHash("sha256").update(resetToken).digest("hex");
